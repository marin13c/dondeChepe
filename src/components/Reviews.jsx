...
function StarRating({ rating }) {
  return (
    <div className="flex gap-0.5">
      {[1, 2, 3, 4, 5].map((s) =>
        s <= rating
          ? <RiStarFill key={s} className="text-[#c47830] text-sm" />
          : <RiStarLine key={s} className="text-[#c47830]/30 text-sm" />
      )}
    </div>
  );
}
...
function InteractiveStars({ value, onChange }) {
  ...
  return (
    <div>
      <div className="flex gap-2 mb-2">
        {[1, 2, 3, 4, 5].map((star) => (
          <button
            ...
            style={{
              color: star <= display ? '#c47830' : '#3d2512',
      ...
      <p className="font-['DM_Sans'] text-xs text-[#f2e4c8]/40 h-4">
...
function ReviewCard({ review, index, isInView, isNew }) {
  ...
  return (
    <motion.div
      ...
      className="bg-[#1e1008] p-7 flex flex-col gap-5 hover:bg-[#28170a] transition-colors duration-300 relative overflow-hidden"
      style={{
        border: review.rating === 5 ? '1px solid rgba(212,168,83,0.3)' : '1px solid #3d2512',
      }}
    >
      {/* 5-star gold accent bar */}
      {review.rating === 5 && (
        <div className="absolute top-0 left-0 w-0.5 h-full bg-[#c47830]" />
      )}
      ...
      {/* Text */}
      <p className="font-['DM_Sans'] text-[#f2e4c8]/65 text-sm leading-relaxed flex-1">
        "{review.comment}"
      </p>

      {/* Footer */}
      <div className="flex items-center justify-between pt-4 border-t border-[#3d2512]">
        <time className="font-['DM_Sans'] text-xs text-[#f2e4c8]/35">{formattedDate}</time>
        <div className="font-['Playfair_Display'] text-[#c47830] text-2xl font-black leading-none opacity-30">
          "
        </div>
      </div>
    </motion.div>
...
export default function Reviews() {
  ...
  return (
    <section id="resenas" className="relative bg-[#140b05] py-28 overflow-hidden">
      {/* Grain texture */}
      <div
      ...
      <div ref={ref} className="relative z-10 max-w-7xl mx-auto px-6 lg:px-10">
        ...
        >
          <div>
            ...
            >
              Lorem Ipsum
              <br />
              <span className="italic text-[#c47830]">Dolor</span>
            </h2>
          </div>

          {displayed.length > 0 && (
            <div className="text-right">
              <div className="font-['Playfair_Display'] font-black text-5xl text-[#c47830] leading-none">
                {(displayed.reduce((a, r) => a + r.rating, 0) / displayed.length).toFixed(1)}
              </div>
              <div className="font-['DM_Sans'] text-xs text-[#f2e4c8]/40 mt-1 tracking-widest uppercase">Lorem ipsum dolor</div>
              <div className="flex justify-end mt-2 gap-0.5">
                {[1,2,3,4,5].map(s => <RiStarFill key={s} className="text-[#c47830] text-sm" />)}
              </div>
            </div>
          )}
        </motion.div>
        ...
        <motion.div
          ...
          className="mb-16"
        >
          <div className="max-w-2xl bg-[#1e1008] border border-[#3d2512] p-8 relative overflow-hidden">
            {/* amber accent bar */}
            <div className="absolute top-0 left-0 w-0.5 h-full bg-[#c8621a]" />

            <h3 className="font-['Playfair_Display'] font-bold text-xl text-[#f2e4c8] mb-6">
              Dejá tu reseña
            </h3>
            ...
            <AnimatePresence mode="wait">
              {submitted ? (
                ...
                <motion.div
                  ...
                  className="flex items-start gap-4 bg-[#140b05] border border-[#4caf7d]/30 p-5"
                >
                  ...
                  <div>
                    ...
                    <p className="font-['DM_Sans'] text-[#f2e4c8]/50 text-sm font-light">
                ...
                >
                  {/* Stars */}
                  <div className="mb-6">
                    <label className="block font-['DM_Sans'] text-xs font-semibold tracking-[0.15em] uppercase text-[#f2e4c8]/40 mb-3">
                      Calificación
                    </label>
                  ...
                  <div className="mb-6">
                    <label
                      ...
                      className="block font-['DM_Sans'] text-xs font-semibold tracking-[0.15em] uppercase text-[#f2e4c8]/40 mb-3"
                    >
                      Comentario
                    </label>
                    <textarea
                      ...
                      rows={4}
                      className="w-full bg-[#140b05] text-[#f2e4c8] font-['DM_Sans'] text-sm font-light placeholder:text-[#f2e4c8]/20 p-4 resize-none outline-none transition-all duration-200"
                      style={{
                        border: commentError ? '1px solid #e05555' : '1px solid #3d2512',
                        boxShadow: commentError ? '' : comment.length > 0 ? '0 0 0 1px #c8621a33' : '',
                      }}
                      onFocus={e => { if (!commentError) e.target.style.borderColor = '#c8621a'; }}
                      onBlur={e  => { if (!commentError) e.target.style.borderColor = '#3d2512'; }}
                    />
                  ...
                  <button
                    type="submit"
                    className="font-['DM_Sans'] font-semibold text-xs tracking-widest uppercase bg-[#c8621a] hover:bg-[#e07a30] text-[#f2e4c8] px-8 py-3.5 transition-all duration-300 hover:shadow-lg hover:shadow-[#c8621a]/30 hover:-translate-y-0.5 active:translate-y-0"
                  >
                    Enviar Reseña →
                  </button>
                </motion.form>
        ...
        <div id="resenas-lista">
          ...
          <motion.div
            ...
            className="flex items-center justify-between mb-8 pb-5 border-b border-[#3d2512]"
          >
            <p className="font-['DM_Sans'] text-sm text-[#f2e4c8]/40">
              <span className="font-['Playfair_Display'] font-bold text-lg text-[#c47830] mr-2">
                {displayed.length}
              </span>
          ...
          {displayed.length === 0 ? (
            <motion.div
              ...
              className="text-center py-16 border border-dashed border-[#3d2512]"
            >
              <div className="font-['Playfair_Display'] text-4xl text-[#c47830]/20 mb-4">★</div>
              <p className="font-['Playfair_Display'] font-bold text-lg text-[#f2e4c8]/30 mb-2">
                Sin reseñas destacadas aún
              </p>
              <p className="font-['DM_Sans'] text-sm text-[#f2e4c8]/20 font-light">
